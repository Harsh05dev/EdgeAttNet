...
## Experiment phases (fair comparison vs U-Net)

Runs are organized under `runs/` in both repos so old records are never lost.
...
| 2 fair prep | `runs/phase2_fair_prep_year512_ep50` (both) | preprocessed | year split (prep-filtered) | **512** | 50 |
| 2b fair prep | `runs/phase2_fair_prep_year2048_ep50` (both) | preprocessed | same prep split | **2048** | 50 |
| 3 fair raw | `runs/phase3_fair_raw_year512_ep50` (both) | raw JPEGs | expanded val split (2018–2020) | **512** | 50 |

Shared split JSONs (identical in both repos): `splits/year_2011-2019_val2020_test2021-2022/`
(train 1412 / val 62 / test 119). Generated by `split_coco_year.py`.

### Phase 1 — matched settings (both models)
...
| test | evaluated once at end on best checkpoint |
...
### Phase 2 — preprocessed images at 512 (complete)

| Setting | Value |
|---------|-------|
| Images | `data/processed-H-alpha/` (paper pipeline; U-Net reads same path) |
| Split | `splits/year_2011-2019_val2020_test2021-2022_prep/` (1322 / 18 / 99) |
| image_size | 512, epochs 50, batch 4, lr 1e-4, Adam, no augmentation |
...
### Phase 3 — raw @ 512 with expanded val (complete)

Train 2011–2017, val 2018–2020 (193 images), test 2021–2022. Same settings as Phase 1 otherwise.

| Model | Test Dice | Test IoU | Best val Dice | Best epoch |
|-------|----------:|---------:|--------------:|-----------:|
| U-Net | 0.601 | 0.430 | 0.611 | 32 |
| EdgeAttNet | **0.611** | **0.440** | **0.622** | 25 |
